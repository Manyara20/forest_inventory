...
const DisabledNumericalField = (props) => {

    const {placeholder, label, name, ifRequired, errors, register}=props;
  ...
  return (
    <div className='my-2'>
        <label className={`${styles.formLabels}`}>{label}</label>
            <input className={customEmailClass}type='text' placeholder={placeholder} 
              {...register(name, { required: {value: ifRequired, message: "Mandatory Field"},
                 })}
                 readOnly/>
    ...
    DisabledNumericalField.propTypes ={
...
export default DisabledNumericalField